    ...
    function actualizarCarrito() {
        ...
        checkboxes.forEach(function (checkbox) {
            if (checkbox.checked) {
                ...
                const precio = 0;
                if (producto=='Recibir Periódicos a Domicilio - $6000.00') {
                    precio = parseFloat(6000);
                }
                total += precio;
        ...
        radios.forEach(function (radio) {
            if (radio.checked) {
                ...
                const precio = 0;
                if (subscripcion=='Subscripción Anual - $36720.00') {
                    precio = parseFloat(36720);
                }else{
                    precio = parseFloat(3600);
                }
                total += precio;
                // Agrega la suscripción al array de productos seleccionados
                productosSeleccionados.push(`${subscripcion}`);
            }
        });
